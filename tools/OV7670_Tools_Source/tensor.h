...
class tensor{
	public:		
		...
		void operator=(tensor A){
			if(this->data==NULL || this->size[2]!=A.size[2]){ this->relloc(A.size[0],A.size[1]); }
...
tensor operator*(tensor A, tensor B){
	...
	if(A.size[0] != B.size[1]){ printf("producto . erroneo (%d,%d) (%d,%d)", A.size[0],A.size[1],B.size[0],B.size[1]); exit(1);}
...
tensor operator+(tensor A, tensor B){
	...
	else{ if(A.size[0] != B.size[0] or A.size[1] != B.size[1]){printf("adicion erronea: (%d,%d) (%d,%d)", A.size[0],A.size[1],B.size[0],B.size[1]); exit(1);}
...
tensor operator-(tensor A, tensor B){
	T_reg[0].relloc(B.size[0],B.size[1]);
	if(A.size[2]==0){ return B; }
	else{ if(A.size[0] != B.size[0] or A.size[1] != B.size[1]){printf("substraccion erronea: (%d,%d) (%d,%d)", A.size[0],A.size[1],B.size[0],B.size[1]); exit(1);}
		  for(int i=A.size[2]; i--;){T_reg[0].data[i] = A.data[i] - B.data[i];}}
	return T_reg[0];}
					
// Tensor math  ##############################################################//
void operator+=(tensor A, tensor B){
	if(A.size[0] != B.size[0] or A.size[1] != B.size[1]){printf("adicion erronea: (%d,%d) (%d,%d)", A.size[0],A.size[1],B.size[0],B.size[1]); exit(1);}
	for(int i=A.size[2]; i--;){A.data[i] += B.data[i];}}
		
void operator-=(tensor A, tensor B){
	if(A.size[0] != B.size[0] or A.size[1] != B.size[1]){ printf("sustraccion erronea: (%d,%d) (%d,%d)", A.size[0],A.size[1],B.size[0],B.size[1]); exit(1);}
	for(int i=A.size[2]; i--;){A.data[i] -= B.data[i];}}
			
// Tensor math with point to point ###########################################//
tensor operator>(tensor A, tensor B){
	T_reg[2].relloc(A.size[0],A.size[1]);
	if(A.size[0] != B.size[0] or A.size[1] != B.size[1]){ printf("producto X erronea: (%d,%d) (%d,%d)", A.size[0],A.size[1],B.size[0],B.size[1]); exit(1);}
...
//TODO: Image Processing #####################################################//
	...
	return I_reg[3];} //FIXME: 255 image normalization
...
// FIXME: Create a Tensor increase_image(){};
	...
	tensor convolution(tensor A, tensor B){
		...
		if(A.size[2] < B.size[2]){printf("error en la correlacion (%d,%d)*(%d,%d)", A.size[0], 	A.size[1], B.size[0],	B.size[1]); exit(1);}
		
		for(int i=I_reg[3].size[1]; i--;){for(int j=I_reg[3].size[0]; j--;){ for(int k=B.size[1]; k--;){for(int l=B.size[0]; l--;){
			I_reg[3].data[I_reg[3].get_index(i,j)] += A.data[A.get_index(i+k,j+l)] * B.data[B.get_index(l,k)];
		}}}}
	...
	tensor correlation(tensor A, tensor B){
		...
		for(int i=I_reg[3].size[1]; i--;){for(int j=I_reg[3].size[0]; j--;){ for(int k=B.size[1]; k--;){for(int l=B.size[0]; l--;){
			I_reg[3].data[I_reg[3].get_index(i,j)] += A.data[A.get_index(i+l,j+k)] * B.data[B.get_index(k,l)];
		}}}}
	...
	tensor get_filter(int type){
		...
		switch(type){
			case gauss: size=2; index=0; break;

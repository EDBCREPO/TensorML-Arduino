...
tensor bnr(tensor X, bool dev){
	T_reg[5].relloc(X.size[0],X.size[1]);
	if(dev==true){ for(int i=X.size[2]; i--;){
		if( X.data[i] > 1 )
			T_reg[5].data[i]=1;
		else if( X.data[i] < -1 )
			T_reg[5].data[i]=-1;
		else
			T_reg[5].data[i]=X.data[i];
	}}
	else{ for(int i=X.size[2]; i--;){
...
tensor activation(tensor X, int kernel, bool dev){
	switch(kernel){
		...
		case binary: 	return(bnr(X,dev));break;
